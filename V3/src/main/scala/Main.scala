...
object Main extends App {
  ...
  type EvaluateEnvironmentFunctionPartial = Environment ?=> Expression
  ...
  def eval(expression: Expression): EvaluateEnvironmentFunction = expression match {
    case Value(v) => v
    case Variable(someName) => if(getscopepartial().getOrElse(someName, throw Exception(s"undefined name $Scopes")).isInstanceOf[Int])
      {
        getscopefull().getOrElse(someName, throw Exception(s"undefined name $Scopes"))
      }
    else
      {
        eval(getscopepartial().getOrElse(someName, throw Exception(s"undefined name $Scopes")).asInstanceOf[Expression])
      }
    ...
    case Assign(variable, o1) => eval(Assignhelper(variable, o1))
  ...
  def getscopefull(): scala.collection.mutable.Map[String, Int] = {
    val temp: scala.collection.mutable.Map[String, Int] = scala.collection.mutable.Map.empty
    if (Scopes.isEmpty) {
  ...
  //gets the scope so assign and Variable() expressions can go inside the scope and change variables to expressions
  def getscopepartial(): scala.collection.mutable.Map[String, Any] = {
    val temp: scala.collection.mutable.Map[String, Any] = scala.collection.mutable.Map.empty
    if (Scopes.isEmpty) {
      //scope is empty so make scope the environment and return it
      Scopes ++= summon[Environment].asInstanceOf[scala.collection.mutable.Map[String, Any]]
      Scopes.asInstanceOf[scala.collection.mutable.Map[String, Any]]
    }
    else if (scope == "") {
      //scope is empty so make scope the environment and return it
      Scopes ++= summon[Environment].asInstanceOf[scala.collection.mutable.Map[String, Any]]
  ...
  //helper function for eval assign
  def Assignhelper(variable: Variable, o1: Expression): Expression = {
    //call getscope to get the scope correctly then assign the eval(o1) to the variable 
    getscopefull()(variable.name) = eval(o1)
    o1
  }
  //helper function for partial assign
  def PartialAssignhelper(variable: Variable, o1: Expression): Expression = {
    //call getscope to get the scope correctly then assign the eval(o1) to the variable 
    getscopepartial()(variable.name) = o1
    o1
  }
  ...
  class classInstance (name: String) {
    ...
    def InvokeMethod(methodname: String, parameters: List[(String, Any)]): Any = {
      ...
      //method methodname can't be found inside class
      println("Can't find method")
      //end
      return returnvar
    }
    //InvokePartialMethod definition - it first searches for the class called name then it checks if the class is derived 
    //from any other class if it then it copies the methods from the base class and adds it to the list of methods inside
    //the derived class. Then it searches for the method methodname inside the list of methods and does every expression partially and
    //only returns the partial expression of the last expression contained in the method.
    def InvokePartialMethod(methodname: String, parameters: List[(String, Any)]): Any = {
      var returnvar: Any = 0
      var methodlistsize = 0
      var Class: scala.collection.mutable.Map[String, ListBuffer[Any]] = getmyclass(classes, classname)
      //copy methods from base class if class extends base class
      ...
      for (methodlistsize <- 0 to Class(classname)(1).asInstanceOf[ListBuffer[Any]].length-1) {
        ...
        if (Class(classname)(1).asInstanceOf[ListBuffer[scala.collection.mutable.Map[String, ListBuffer[Any]]]](methodlistsize).contains(methodname)) {
          ...
          for (i <- 0 to Class(classname)(1).asInstanceOf[ListBuffer[scala.collection.mutable.Map[String, List[List[Expression]]]]](methodlistsize)(methodname)(1).length-1) {
            //get to last expression in expression list
            returnvar = Partial((Class(classname)(1).asInstanceOf[ListBuffer[scala.collection.mutable.Map[String, List[List[Expression]]]]](methodlistsize)(methodname)(1)(i)).asInstanceOf[Expression])
          }
          //returns the value of the last expression contained in the method
          //end
          return returnvar
  ...
  def Partial(partialExp:Expression): EvaluateEnvironmentFunctionPartial = partialExp match{
    case Value(v) => partialExp
    case Variable(someName) => if(getscopepartial().getOrElse(someName, throw Exception(s"undefined name $Scopes")).isInstanceOf[Expression]){
      return getscopepartial().getOrElse(someName, throw Exception(s"undefined name $Scopes")).asInstanceOf[Expression]
    }
    else{
      return partialExp
    }
    case Add(o1, o2) =>
      ...
      if(left.isInstanceOf[Value] && right.isInstanceOf[Value]){
    ...
    case Multiply(o1, o2) =>
      ...
      Multiply(left,right)
    case Assign(variable, o1) => PartialAssignhelper(variable, o1)
    case Let(variable) => partialExp
  }

  var istrue: Boolean = false

  def IFTRUE(result: Boolean): Boolean = {
    if (result == true) {
      istrue = true
      return istrue
    }
  ...
  implicit class THENEXECUTE(fromif: Boolean) extends AnyVal{
    def THENEXECUTE(expression: Expression): Expression  = {
      //        println(expression)
      return expression
    }
  }
  implicit class ELSERUN(fromthenexecute: Expression) extends AnyVal {
    def ELSERUN(expression: Expression): Expression = {
      if (istrue == false) {
        println("in else run")
        return expression
      }
        ...
        println("in else run")
        //eval(fromthenexecute)
        return fromthenexecute
      }
    }
  }

  def GREATEREQUAL(expression1: Expression, expression2: Expression): Boolean = {
  ...
  def LESSEREQUAL(expression1: Expression, expression2: Expression): Boolean = {
    if(eval(expression1)<=eval(expression2)) {
      return true;
    }
    else
  ...
  def EQUALEQUAL(expression1: Expression, expression2: Expression): Boolean = {
    if(eval(expression1)==eval(expression2)) {
      return true;
    }
    else
  ...
  var test = 0
  eval(Assign(Variable("var"), Value(4)))
  eval(Assign(Variable("var2"), Value(4)))
  eval(RUN(
    ()=>{
      test = test + 1
    }
  ...
  println(test)
  println(Scopes)
  //assign variable var3 to partial multiply
  println(Partial(Assign(Variable("var3"), Partial(Multiply(Value(3), Multiply(Value(5), Variable("var")))))))
  //check actual variable
  println(eval(Variable("var3")))
  //check partial variable
  println(Partial(Variable("var3")))
  println(Partial(Add(Value(2),Value(4))))
  Class("Base") 
  Method(Class("Base"), MethodName("m1"), List(Parameter("p1", ParamType("int")), Parameter("p2", ParamType("string"))), List(Multiply(Value(3), Multiply(Value(5), Variable("var"))), Multiply(Value(3), Multiply(Value(5), Variable("var")))))
